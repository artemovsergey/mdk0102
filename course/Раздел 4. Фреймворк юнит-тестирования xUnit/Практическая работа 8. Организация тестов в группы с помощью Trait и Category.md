...
5. Настройте фильтрацию тестов: команда `dotnet test --filter` и фильтры панели Testing VS Code
...
- Сделаны скриншоты фильтрации тестов (панель Testing или вывод `dotnet test --filter`)
...
4. Как выполнить только тесты определенной категории? Приведите команду `dotnet test --filter`.
