...
1.  Создайте решение с двумя проектами в терминале VS Code (Ctrl + `):
    ```bash
    dotnet new classlib -n StringCalculatorLib
    dotnet new xunit -n StringCalculatorLib.Tests
    dotnet new sln -n StringCalculator
    dotnet sln add StringCalculatorLib StringCalculatorLib.Tests
    dotnet add StringCalculatorLib.Tests reference StringCalculatorLib
    ```
    *   **Проект приложения (Class Library):** `StringCalculatorLib`
    *   **Тестовый проект (xUnit Test Project):** `StringCalculatorLib.Tests`
2.  Проверьте, что тестовый проект ссылается на проект приложения (ссылка добавлена командой `dotnet add reference` на шаге 1).
...
6.  Убедитесь, что все тесты проходят: выполните `dotnet test` в терминале или нажмите «Run All Tests» в панели **Testing** VS Code.
7.  Создайте папку `images` в корне репозитория и добавьте в нее скриншот панели Testing (или вывода `dotnet test`) с результатами успешного прохождения всех ваших тестов.
...
├── images/
│   └── test_results.png (скриншот панели Testing или вывода dotnet test)
└── README.md
```
