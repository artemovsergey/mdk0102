...
1. Создать проект **xUnit Test Project (.NET)**: `dotnet new xunit -n LocatorTests` (или Create .NET Project → xUnit в VS Code).
2. Установить через NuGet:

   * `Selenium.WebDriver` 4.49.0
   * `Selenium.Support` 4.49.0
   * Драйвер браузера отдельным пакетом не устанавливается: Selenium Manager скачает его автоматически
...
{
    public class LocatorExamples : IDisposable
    {
        private readonly IWebDriver _driver;

        public LocatorExamples()
        {
...
public void FindElementsUsingDifferentLocators()
{
    ...
    Assert.True(usernameById.Displayed, "Элемент по ID не найден");

    // Name
    IWebElement passwordByName = _driver.FindElement(By.Name("password"));
    Assert.True(passwordByName.Displayed, "Элемент по Name не найден");

    // ClassName
    IWebElement loginButtonByClass = _driver.FindElement(By.ClassName("btn-primary"));
    Assert.True(loginButtonByClass.Displayed, "Элемент по ClassName не найден");

    // CSS Selector
    IWebElement loginByCss = _driver.FindElement(By.CssSelector("#loginBtn"));
    Assert.True(loginByCss.Displayed, "Элемент по CSS Selector не найден");

    // XPath
    IWebElement linkByXPath = _driver.FindElement(By.XPath("//a[contains(text(),'Forgot Password')]"));
    Assert.True(linkByXPath.Displayed, "Элемент по XPath не найден");
}
```

### Шаг 4. Запуск теста

1. Выполнить `dotnet test` (или Run All Tests в панели Testing VS Code).
...
Assert.True(username.Displayed, "Поле логина не найдено");
````
