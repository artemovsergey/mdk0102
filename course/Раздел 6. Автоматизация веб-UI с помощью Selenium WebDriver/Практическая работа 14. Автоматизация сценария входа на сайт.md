...
1. Создать проект **xUnit Test Project** на C# (`dotnet new xunit -n LoginTests`).
2. Установить через NuGet:

   * `Selenium.WebDriver` 4.49.0
   * `Selenium.Support` 4.49.0
   * `DotNetSeleniumExtras.WaitHelpers` 3.11.0 (для `ExpectedConditions`)
...
public class LoginTests : IDisposable
{
    ...
    public void SuccessfulLoginTest()
    {
        ...
        Assert.True(_driver.Url.Contains("/home"), "Переход на главную страницу не выполнен");
    }
...
* Выполнить `dotnet test` (или Run All Tests в панели Testing VS Code)
...
6. Где в xUnit размещается код подготовки и очистки теста?
...
Assert.True(usernameField.Displayed, "Поле Username не отображается");
````
